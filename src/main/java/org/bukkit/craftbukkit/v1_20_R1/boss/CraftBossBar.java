...
public class CraftBossBar implements BossBar {
    ...
    public String getTitle() {
        return CraftChatMessage.fromComponent(handle.getName()); // CatServer - name -> getName()
    }

    @Override
    public void setTitle(String title) {
        handle.setName(CraftChatMessage.fromString(title, true)[0]); // CatServer - name -> setName()
        handle.broadcast(ClientboundBossEventPacket::createUpdateNamePacket);
    }

    @Override
    public BarColor getColor() {
        return convertColor(handle.getColor()); // CatServer - color -> getColor()
    }

    @Override
    public void setColor(BarColor color) {
        handle.setColor(convertColor(color)); // CatServer - color -> setColor()
        handle.broadcast(ClientboundBossEventPacket::createUpdateStylePacket);
    }

    @Override
    public BarStyle getStyle() {
        return convertStyle(handle.getOverlay()); // CatServer - overlay -> getOverlay()
    }

    @Override
    public void setStyle(BarStyle style) {
        handle.setOverlay(convertStyle(style)); // CatServer - overlay -> setOverlay()
        handle.broadcast(ClientboundBossEventPacket::createUpdateStylePacket);
    }
    ...
    public boolean isVisible() {
        return handle.isVisible(); // CatServer - visible -> isVisible()
    }
