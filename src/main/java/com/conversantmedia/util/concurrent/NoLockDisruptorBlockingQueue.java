 ...
 */
public final class NoLockDisruptorBlockingQueue<E> extends MultithreadConcurrentQueue<E> implements Serializable, Iterable<E>, Collection<E>, BlockingQueue<E>, Queue<E> {
    ...
    public NoLockDisruptorBlockingQueue(final int capacity) {
     ...
     */
    public NoLockDisruptorBlockingQueue(final int capacity, final SpinPolicy spinPolicy) {
        super(capacity);
     ...
     */
    public NoLockDisruptorBlockingQueue(final int capacity, Collection<? extends E> c) {
        this(capacity);
        for (final E e : c) {
            offer(e);
        }
    ...
    private final class RingIter implements Iterator<E> {
        ...
        @Override
        public void remove() {
            NoLockDisruptorBlockingQueue.this.remove(lastObj);
        }
    }
