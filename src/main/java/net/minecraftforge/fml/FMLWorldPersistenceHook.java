...
public final class FMLWorldPersistenceHook implements WorldPersistenceHooks.WorldPersistenceHook
{
    ...
    private static boolean injectedSnapshot = false; // CatServer - Don't inject plugin world snapshot
    ...
    public void readData(SaveFormat.LevelSave levelSave, IServerConfiguration serverInfo, CompoundNBT tag)
    {
        ...
        if (tag.contains("Registries") && !injectedSnapshot /* CatServer - Don't inject plugin world snapshot*/) // 1.8, genericed out the 'registries' list
        {
            ...
            injectedSnapshot = true; // CatServer
        }
