...
public class ProxyMethodHandle {
    ...
    static {
        try {
            ProxyMethodHandle.loadMappings(new BufferedReader(new InputStreamReader(MappingLoader.class.getClassLoader().getResourceAsStream("mappings/spigot2srg.srg"))));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
